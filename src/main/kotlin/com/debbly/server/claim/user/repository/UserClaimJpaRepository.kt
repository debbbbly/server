...
interface UserClaimJpaRepository : JpaRepository<UserClaimEntity, UserClaimId> {
    ...
    @Query("SELECT uc.id.claimId as claimId, uc.stance as stance, COUNT(uc) as count FROM users_claims uc WHERE uc.id.claimId IN :claimIds GROUP BY uc.id.claimId, uc.stance")
